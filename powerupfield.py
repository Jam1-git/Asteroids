...
class PowerupField(pygame.sprite.Sprite):
    ...
    def spawn(self, position, velocity, power_type, duration):
        powerup = Powerup(position.x, position.y, power_type, duration)
    ...
    def update(self, dt):
        ...
        if self.spawn_timer > POWERUP_SPAWN_RATE:
            ...
            power_type, duration = random.choice(Powerup.POWERS)
            self.spawn(position, velocity, power_type, duration)